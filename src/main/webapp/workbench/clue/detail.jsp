...
<script type="text/javascript">
		...
		$.ajax({
			...
			success:function(data){
				...
				$.each(data, function(index, activity){
					...
					html += '<td>'+activity.endDate+'</td>';
					html += '<td>'+activity.owner+'</td>';
					// 这行关联了一个该线索需要删除的关联的市场活动信息的操作(解除关联)
					// 对于动态生成的标签想要触发事件，最好使用事件绑定的机制
					// 向里面传递参数需要特别注意
					// 这里的activity.id已经在显示该列表的时候在DAO层被保存为tbl_clue_activity_relation中的id了
					// 设想的是根据关联关系表的id删除单条记录
					html += '<td><a href="javascript:void(0);"  onclick="unlocate(\''+activity.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
				});

				// 将动态生成的市场活动列表添加到body中
	...
	function unlocate(id){
		// 根据id删单条
		$.ajax({
			url:"workbench/clue/unlocate.do",
			data:{
				"id":id
			},
			dataType:"json",
...
<%--							</tr>--%>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/convert.jsp?flag=a&clueId=${requestScope.clue.id}&fullname=${requestScope.clue.fullname}&appellation=${requestScope.clue.appellation}&company=${requestScope.clue.company}&owner=${requestScope.clue.owner}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
